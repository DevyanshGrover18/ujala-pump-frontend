...
export function usePendingPayoutCount() {
  ...
  const fetchCount = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/payouts/pending-count`,
        {
  ...
  useEffect(() => {
    ...
    window.addEventListener('payouts-updated', handleUpdate);
    window.addEventListener('focus', handleUpdate);

    return () => {
      clearInterval(interval);
      window.removeEventListener('payouts-updated', handleUpdate);
