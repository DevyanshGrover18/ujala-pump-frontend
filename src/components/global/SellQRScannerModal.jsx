...
function SellQRScannerModal({ isOpen, onClose, onProductScanned }) {
    ...
    try {
      if (videoRef.current) {
        qrScannerRef.current = new QrScanner(
          videoRef.current,
          (result) => {
            // Normalize and defensively extract serial values from QR payloads
            try {
              let raw = result.data;
              let candidate;
              try {
                ...
                candidate =
                  parsed.serialNumber || parsed.serial || parsed.sn || raw;
              } catch {
                candidate = raw;
              }
              ...
              const normalize = (s) => {
                ...
                // remove common control / unicode BOM chars and trim
                return String(s)
                  .replace(/[\u0000-\u001F\u007F-\u009F\uFEFF]/g, '')
                  .trim();
              };
  ...
  const fetchProductDetails = async (serialNumber) => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}/api/products/serial/${encodeURIComponent(normalized)}`,
        {
          headers: {
      ...
      const normalizedErrKey = String(serialNumber)
        .replace(/[\u0000-\u001F\u007F-\u009F\uFEFF]/g, '')
        .trim();
      // Avoid spamming the same error toast for duplicate scans
      if (lastErrorRef.current !== normalizedErrKey) {
        lastErrorRef.current = normalizedErrKey;
  ...
  const handleManualSubmit = () => {
    const normalized = String(manualInput || '')
      ...
      .trim();
    if (normalized) {
      // reset last error to allow immediate feedback for manual input
      lastErrorRef.current = null;
      fetchProductDetails(normalized);
    }
  };
  ...
  useEffect(() => {
    ...
    return () => {
      // cleanup timers and scanner if component unmounts
      if (scanTimeoutRef.current) clearTimeout(scanTimeoutRef.current);
      if (qrScannerRef.current) {
        qrScannerRef.current.destroy();
        qrScannerRef.current = null;
...
export default SellQRScannerModal;
