...
export default function AddMemberForm() {
  ...
  const [accessControl, setAccessControl] = useState({
    ...
    subDealers: { add: false, modify: false, delete: false, full: false },
  });
      ...
      {activeTab === 'privileges' && (
        <div className="space-y-4">
          <p className="text-gray-600 mb-4">
            Configure access permissions for this member across different
            dashboard sections.
          </p>
                ...
                {sections.map((section) => (
                  <tr key={section} className="hover:bg-gray-50">
                    ...
                    {['add', 'modify', 'delete', 'full'].map((permission) => (
                      <td
                        key={permission}
                        className="px-4 py-3 whitespace-nowrap text-center text-sm text-gray-500"
                      >
                        {section === 'sales' || section === 'products' ? (
                          // For sales and products sections
                          permission === 'full' ? (
                            <input
                              type="checkbox"
                              checked={accessControl[section].full}
                              onChange={() =>
                                handleAccessControlChange(section, 'full')
                              }
                              className="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 cursor-pointer"
                            />
                          ) : (
                            <span className="text-gray-400">-</span> // Placeholder for add/modify/delete
                          )
                        ) : (
                          // For other sections
                          <input
