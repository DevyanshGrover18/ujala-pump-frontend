...
function Orders() {
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    const matchesSearch =
      ...
      (order.factory &&
        order.factory.name.toLowerCase().includes(lowerCaseSearchTerm)) ||
      (order.orderId &&
        order.orderId.toLowerCase().includes(lowerCaseSearchTerm)) ||
      (order.serialNumber &&
        order.serialNumber.toLowerCase().includes(lowerCaseSearchTerm)) ||
      (order.model &&
        order.model.name.toLowerCase().includes(lowerCaseSearchTerm));

    const matchesFactory =
      factoryFilter === 'all' || order.factory?._id === factoryFilter;
    const matchesOrderType =
      orderTypeFilter === 'all' || order.orderType === orderTypeFilter;
    const matchesDispatched =
      dispatchedFilter === 'all' ||
      String(order.dispatched) === dispatchedFilter;
    const matchesModel =
      modelFilter === 'all' || order.model?._id === modelFilter;
  ...
  const handleAddOrderClick = (isManualOrder = false) => {
    if (factories.length === 0) {
      toast.error('Please wait for factories to load');
      return;
    }
    ...
    return currentItems.length > 0 ? (
      ...
      <div className="text-center py-12">
        ...
        <div className="mt-4 flex justify-center gap-3">
          <button
            onClick={() => handleAddOrderClick(false)}
            className="inline-flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-[#4d55f5] hover:bg-[#3d45e5]"
          >
            Add New Order
          </button>
          <button
            onClick={() => handleAddOrderClick(true)}
            className="inline-flex items-center justify-center px-4 py-2 border border-[#4d55f5] rounded-md shadow-sm text-sm font-medium text-[#4d55f5] bg-white hover:bg-gray-50"
          >
  ...
  return (
    <div className="p-2 sm:p-6 space-y-4">
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        ...
        {/* Table and Cards */}
        <div className="relative min-h-[200px]">{renderContent()}</div>
      ...
      <OrderDetailsModal
        ...
        onStatusChange={async (itemIds, status) => {
          await orderService.bulkUpdateOrderStatus(
            selectedOrder.factory._id,
            itemIds,
            status
          );
          const itemsRes = await orderService.fetchOrderItems(
            selectedOrder._id
          );
          setOrderItems(itemsRes.data);
        }}
