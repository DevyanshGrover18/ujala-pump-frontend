...
export const useOrders = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const [ordersRes, factoriesRes, categoriesRes, modelsRes] =
        await Promise.all([
      ...
      setCategories(
        categoriesRes.data.filter((cat) => cat.status === 'Active')
      );
  ...
  const addOrder = async (orderData, retryCount = 0) => {
    ...
    try {
      ...
      if (
        error.response?.data?.error === 'DUPLICATE_KEY_ERROR' &&
        retryCount < 3
      ) {
        toast.warning(`Duplicate detected, retrying... (${retryCount + 1}/3)`);
        // Wait a short time before retrying
        await new Promise((resolve) =>
          setTimeout(resolve, 500 + retryCount * 200)
        );
  ...
  const deleteMultipleOrders = async (ids) => {
    if (
      !window.confirm(
        `Are you sure you want to delete ${ids.length} selected orders?`
      )
    )
      return false;
