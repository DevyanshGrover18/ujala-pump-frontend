...
export default function Executives() {
  ...
  const fetchDistricts = async (state) => {
    ...
    try {
      const response = await axios.get(
        `${API_URL}/locations/districts/${state}`
      );
  ...
  const fetchLocations = async (state, district) => {
    ...
    try {
      const response = await axios.get(
        `${API_URL}/locations/locations/${state}/${district}`
      );
  ...
  const handleOpenEdit = async (exec) => {
    ...
    const initialAssignments = distIds.map((distId) => {
      const cardDealers = dealerIds.filter((dId) => {
        ...
        const parentDistId =
          typeof dealerObj.distributor === 'object' &&
          dealerObj.distributor !== null
      ...
      const cardSubDealers = subDealerIds.filter((sdId) => {
        ...
        const parentDealerId =
          typeof sdObj.dealer === 'object' && sdObj.dealer !== null
            ? sdObj.dealer._id
            : sdObj.dealer;
  ...
  const handleDelete = async (id) => {
    if (
      window.confirm(
        'Are you sure you want to delete this executive? This will also delete their login account.'
      )
    ) {
      try {
        ...
        toast.error(
          error.response?.data?.message || 'Error deleting executive'
        );
      }
    }
  };
  ...
  const handleSelect = (id) => {
    setSelectedExecutives((prev) =>
      prev.includes(id) ? prev.filter((execId) => execId !== id) : [...prev, id]
    );
  };

  const handleSelectAll = (e) => {
    if (e.target.checked) {
      setSelectedExecutives(filteredExecutives.map((exec) => exec._id));
    } else {
  ...
  const handleDeleteSelected = async () => {
    if (
      window.confirm(
        `Are you sure you want to delete ${selectedExecutives.length} selected executives? This will also delete their login accounts.`
      )
    ) {
      try {
        await axios.delete(`${API_URL}/executives`, {
          data: { executiveIds: selectedExecutives },
        });
        fetchData();
        setSelectedExecutives([]);
        toast.success('Selected executives deleted successfully');
      } catch (error) {
        toast.error(
          error.response?.data?.message || 'Error deleting executives'
        );
      }
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const flatDists = [
      ...new Set(assignments.map((a) => a.distributorId).filter(Boolean)),
    ];
    const flatDealers = [...new Set(assignments.flatMap((a) => a.dealerIds))];
    const flatSubDealers = [
      ...new Set(assignments.flatMap((a) => a.subDealerIds)),
    ];
    ...
    try {
      if (isEditing) {
        await axios.put(
          `${API_URL}/executives/${selectedExecutive._id}`,
          payload
        );
  ...
  const handleStatusToggle = async (exec) => {
    try {
      ...
      toast.success(
        `Executive ${updatedStatus ? 'activated' : 'deactivated'} successfully`
      );
      fetchData();
    } catch (error) {
        ...
        <div className="p-4 lg:p-6 border-b border-gray-200">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
            <div>
              ...
              <p className="text-sm text-gray-600">Total {executives.length}</p>
            </div>
            <div className="flex flex-col lg:flex-row items-stretch lg:items-center space-y-3 lg:space-y-0 lg:space-x-4">
              ...
              {selectedExecutives.length > 0 ? (
                ...
                >
                  ...
                  <span>Delete ({selectedExecutives.length})</span>
                </button>
              ) : (
                <button
              ...
              <div className="flex justify-end mb-4 px-4 sm:px-6">
                <TableExportButtons
                  ...
                  exportData={filteredExecutives.map((e) => ({
                    ...
                    'Assigned Distributors': e.distributors
                      .map((d) => d.name)
                      .join(', '),
                    'Assigned Dealers': e.dealers.map((d) => d.name).join(', '),
                    'Assigned Sub Dealers': e.subDealers
                      .map((sd) => sd.name)
                      .join(', '),
                  }))}
                />
                      ...
                      <tr key={exec._id} className="hover:bg-gray-50/50">
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                          <input
                            ...
                            onChange={() => handleSelect(exec._id)}
                          />
                        </td>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      handleDistrictChange(formData.state, e.target.value)
                    }
                ...
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Configure independent distributor hierarchies for this
                      Executive.
                    </p>
                  </div>
                  <button
                ...
                <div className="space-y-4">
                  {assignments.map((card, index) => {
                    ...
                    const cardSubDealersOptions = subDealers.filter((sd) => {
                      const sdDealerId =
                        typeof sd.dealer === 'object' && sd.dealer !== null
                          ? sd.dealer._id
                          : sd.dealer;
                    ...
                    const searchedCardDealers = cardDealersOptions.filter((d) =>
                      d.name
                        .toLowerCase()
                        .includes(curDealerSearch.toLowerCase())
                    );

                    const searchedCardSubDealers = cardSubDealersOptions.filter(
                      (sd) =>
                        sd.name
                          .toLowerCase()
                          .includes(curSubDealerSearch.toLowerCase())
                    );

                    return (
                      <div
                        key={index}
                        className="bg-white rounded-xl border border-gray-200 p-4 space-y-4 shadow-sm relative hover:border-gray-300 transition-colors"
                      >
                        {/* Card Header */}
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                          ...
                          <div className="space-y-2">
                            <label className="block text-xs font-bold text-gray-700 uppercase tracking-wider">
                              Dealers ({card.dealerIds.length} Selected)
                            </label>
                            <input
                              type="text"
                            ...
                            <div className="max-h-36 overflow-y-auto border border-gray-150 rounded-lg p-2 space-y-1 bg-gray-50/50">
                              {!card.distributorId ? (
                                ...
                                searchedCardDealers.map((dealer) => {
                                  const isChecked = card.dealerIds.includes(
                                    dealer._id
                                  );
                                  return (
                                    <div
                                    ...
                                    >
                                      {isChecked ? (
                                        <CheckSquare className="w-3.5 h-3.5 text-[#4d55f5]" />
                                      ) : (
                          ...
                          <div className="space-y-2">
                            <label className="block text-xs font-bold text-gray-700 uppercase tracking-wider">
                              Sub Dealers ({card.subDealerIds.length} Selected)
                            </label>
                            <input
                              type="text"
                            ...
                            <div className="max-h-36 overflow-y-auto border border-gray-150 rounded-lg p-2 space-y-1 bg-gray-50/50">
                              {card.dealerIds.length === 0 ? (
                                <div className="text-[11px] text-gray-400 text-center py-4">
                                  Select at least one dealer first
                                </div>
                              ) : (
                                searchedCardSubDealers.map((sd) => {
                                  const isChecked = card.subDealerIds.includes(
                                    sd._id
                                  );
                                  return (
                                    <div
                                      key={sd._id}
                                      onClick={() =>
                                        handleCardSubDealerChange(index, sd._id)
                                      }
                                      className="flex items-center gap-2 p-1 rounded hover:bg-white cursor-pointer select-none text-xs"
                                    >
                                      {isChecked ? (
                              ...
                              {card.dealerIds.length > 0 &&
                                searchedCardSubDealers.length === 0 && (
                                  <div className="text-[11px] text-gray-400 text-center py-2">
