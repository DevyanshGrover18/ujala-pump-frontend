...
const BulkSaleModal = ({ isOpen, onClose, fetchSalesData, distributors }) => {
  ...
  const handleValidateProduct = async (scannedResult) => {
    ...
    try {
      ...
      const { data: product } = await axios.get(
        `${API_URL}/api/qr/admin/${cleanSerial}`
      );
      ...
      setLoading(false);
      // CRITICAL FIX: No setTimeout - unlock immediately
      isProcessing.current = false;
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await axios.post(`${API_URL}/api/sales/bulk-dispatch`, {
                        ...
                        <td className="p-3 text-center">
                          <button
                            onClick={() => {
                              ...
                              // CRITICAL FIX: Remove from scannedSet when deleting
