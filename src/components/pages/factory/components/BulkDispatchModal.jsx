...
const BulkDispatchModal = ({
  ...
  const handleValidateProduct = async (scannedResult) => {
    ...
    try {
      ...
      const { data: product } = await axios.get(
        `${API_URL}/api/qr/${cleanSerial}`
      );

      if (
        currentFactoryId &&
        product.factory?.id &&
        String(product.factory.id) !== String(currentFactoryId)
      ) {
        toast.error('Product belongs to a different factory!');
        isProcessing.current = false;
        return;
      }

      if (product.status !== 'Completed' && product.status !== 'Pending') {
        toast.error(
          `Product status is ${product.status}. Only 'Pending' or 'Completed' products can be dispatched.`
        );
        isProcessing.current = false;
        return;
      }
  ...
  const handleSubmit = async () => {
    if (scannedItems.length === 0)
      return toast.error('No products to dispatch');

    setSubmitting(true);
    try {
      await axios.patch(
        `${API_URL}/api/factories/${currentFactoryId}/orders/bulk-status`,
        {
          itemIds: scannedItems.map((item) => item.id),
          status: 'Dispatched',
        }
      );
      ...
      toast.error(
        error.response?.data?.message || 'Failed to dispatch products'
      );
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-md p-0 sm:p-4">
      <div className="bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl w-full max-w-6xl h-[95vh] sm:h-[85vh] flex flex-col overflow-hidden">
        {/* Header */}
        <div className="px-6 py-3 border-b border-gray-200 flex justify-between items-center shrink-0 bg-white z-10">
        ...
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
          ...
          <div className="flex-1 flex flex-col min-h-0 bg-white">
            <div className="px-6 py-2 flex justify-between items-center bg-slate-50 shrink-0">
