...
export default function Payouts() {
  ...
  const fetchPayouts = useCallback(async () => {
    ...
    try {
      ...
      if (roleFilter !== 'All') params.requesterType = roleFilter;
    ...
    const fetchMembersForRole = async () => {
      try {
        ...
        payouts
          ...
          .forEach((p) => {
            const key = p.requester ? String(p.requester) : p.requesterName;
  ...
  const handleSelectAll = () => {
    const pageIds = paginatedPayouts.map((p) => p._id);
  ...
  const handleDeleteSelected = async () => {
    ...
    if (
      !window.confirm(
        `Are you sure you want to delete ${selectedIds.length} selected payout request(s)? Pending requests will be refunded to the user's wallet.`
      )
    )
      return;
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`${API}/api/payouts`, {
        headers: { Authorization: `Bearer ${token}` },
        data: { ids: selectedIds },
      });
      toast.success('Selected payout requests deleted');
      setSelectedIds([]);
      fetchPayouts();
    } catch (err) {
      console.error('Error deleting payouts:', err);
      toast.error(err.response?.data?.message || 'Failed to delete payout requests');
    }
  };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Review and process seller & plumber incentive withdrawal requests.
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-white hover:bg-gray-50 text-gray-700 font-bold rounded-xl border border-gray-200 shadow-sm transition-all text-sm"
          >
          ...
          <button
            ...
            disabled={loading}
            className="p-2.5 bg-white hover:bg-gray-50 text-gray-600 rounded-xl border border-gray-200 shadow-sm transition-all disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        >
          <div>
            ...
            <p className="text-2xl sm:text-2xl font-bold">{stats.totalCount}</p>
          </div>
          <p className="text-xs text-white/80 mt-2 font-medium">
            Total ₹{stats.totalRequestedAmount.toLocaleString('en-IN')} requested
          </p>
        </div>
        ...
        >
          ...
          <p className="text-xs text-white/80 mt-2 font-medium">
            {stats.pendingCount} requests awaiting verification
          </p>
        </div>
        ...
        >
          ...
          <p className="text-xs text-white/80 mt-2 font-medium">
            {stats.approvedCount} payouts verified & paid
          </p>
        </div>
        ...
        >
          <div>
            ...
            <p className="text-2xl sm:text-2xl font-bold">{stats.rejectedCount}</p>
          </div>
          <p className="text-xs text-white/80 mt-2 font-medium">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 flex-1 flex-wrap">
            ...
            <div className="relative flex-1 min-w-[200px] max-w-sm">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-purple-500"
              />
            </div>
            ...
            <select
              ...
              className="px-3 py-2 border border-gray-200 rounded-xl text-xs bg-white font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-purple-500"
            >
            ...
            <select
              ...
              className={`px-3 py-2 border border-gray-200 rounded-xl text-xs font-medium focus:outline-none focus:ring-2 focus:ring-purple-500 min-w-[170px] ${
                roleFilter === 'All'
            ...
            {!isReadOnly && selectedIds.length > 0 && (
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-xs font-bold rounded-xl shadow-sm transition-all"
              >
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-xs text-gray-400 mt-3 font-medium">Loading requests...</p>
          </div>
          ...
          <div className="text-center py-16 px-4 text-gray-400">
            ...
            <p className="text-sm font-bold text-gray-600">No payout requests found</p>
            ...
            <table className="w-full text-left border-collapse">
                ...
                <tr className="border-b border-gray-100 bg-gray-50/50 text-[11px] font-bold text-gray-400 uppercase tracking-wider">
                  {!isReadOnly && (
                    <th className="py-3.5 px-5 w-10">
                      <input
                        type="checkbox"
                        checked={paginatedPayouts.length > 0 && paginatedPayouts.every((p) => selectedIds.includes(p._id))}
              ...
              <tbody className="divide-y divide-gray-50 text-sm text-gray-700">
                {paginatedPayouts.map((payout) => {
                  ...
                  return (
                    <tr key={payout._id} className={`hover:bg-gray-50/60 transition-colors ${selectedIds.includes(payout._id) ? 'bg-purple-50/50' : ''}`}>
                      {!isReadOnly && (
                      ...
                      {/* Requester */}
                      <td className="py-4 px-5">
                        <div className="font-bold text-gray-900">{payout.requesterName}</div>
                      ...
                      <td className="py-4 px-5">
                        <span
                          className={`px-2.5 py-0.5 rounded-full text-xs font-bold border ${
                            ROLE_BADGE[payout.requesterType] || 'bg-gray-100 text-gray-600'
                          }`}
                        >
                        ...
                        {payout.payoutMethod === 'UPI' ? (
                          ...
                          <div className="text-xs">
                            <div className="font-semibold text-gray-800 flex items-center gap-1">
                              ...
                              <span>{payout.bankDetails?.bankName || 'Bank Account'}</span>
                            </div>
                      ...
                      <td className="py-4 px-5 text-center">
                        ...
                        >
                          ...
                          <span>
                            {payout.status === 'Pending' ? 'Process' : 'Details'}
                          </span>
                        </button>
                      </td>
                    </tr>
              ...
              {!loading && filteredPayouts.length > 0 && (
                <tfoot className="bg-gray-50 border-t-2 border-gray-200 text-xs font-bold text-gray-900">
                  <tr>
                    ...
                    <td colSpan={4} className="py-3 px-5 text-gray-500 font-normal">
                      {selectedMemberName ? `For ${selectedMemberName}` : roleFilter !== 'All' ? `For all ${roleFilter}s` : 'Across all requests'}
                    </td>
                  </tr>
                </tfoot>
              )}
        ...
        {totalPages > 1 && (
          <div className="flex flex-col sm:flex-row justify-between items-center gap-3 px-5 py-4 border-t border-gray-100 text-xs text-gray-500">
            <span>
              ...
              </span> of <span className="font-bold text-gray-700">{filteredPayouts.length}</span> requests
            </span>
            <div className="flex items-center gap-2">
              <button
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden border border-gray-200">
        <div className="flex items-center justify-between p-5 border-b border-gray-100">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-lg font-bold text-gray-900">Minimum Payout Thresholds</h2>
              <p className="text-xs text-gray-500">Set the minimum amount required to raise a withdrawal</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => setForm({ ...form, distributorMinPayout: e.target.value })}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setForm({ ...form, dealerMinPayout: e.target.value })}
                className="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm font-bold focus:ring-2 focus:ring-purple-500"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setForm({ ...form, subDealerMinPayout: e.target.value })}
                className="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm font-bold focus:ring-2 focus:ring-purple-500"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setForm({ ...form, plumberMinPayout: e.target.value })}
                className="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm font-bold focus:ring-2 focus:ring-purple-500"
              />
            </div>
          </div>
...
function ProcessPayoutModal({ payout, onClose, onSuccess }) {
  ...
  const handleProcess = async (e) => {
    ...
    if (action === 'approve' && !proofImage) {
      return toast.error('Payment proof screenshot/receipt is required to approve');
    }
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-xl max-h-[90vh] overflow-y-auto border border-gray-200">
        <div className="flex items-center justify-between p-5 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-500">Requested on {new Date(payout.requestedAt).toLocaleString('en-IN')}</p>
          </div>
          <button onClick={onClose} className="p-1.5 hover:bg-gray-100 rounded-lg text-gray-400">
            <X className="w-5 h-5" />
          </button>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="bg-purple-50/50 border border-purple-100 rounded-2xl p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            <div>
              <div className="flex items-center gap-2">
                <span className="font-bold text-gray-900 text-base">{payout.requesterName}</span>
            ...
            {payout.payoutMethod === 'UPI' ? (
              ...
              <div className="grid grid-cols-2 gap-3 text-xs bg-gray-50 p-3 rounded-xl">
                <div>
                  ...
                  <span className="font-bold text-gray-800">{payout.bankDetails?.bankName || 'N/A'}</span>
                </div>
                <div>
                  <span className="text-gray-400 block font-medium">Account Holder</span>
                  <span className="font-bold text-gray-800">{payout.bankDetails?.accountHolderName || 'N/A'}</span>
                </div>
                <div>
                  <span className="text-gray-400 block font-medium">Account Number</span>
                  <span className="font-mono font-bold text-gray-900">{payout.bankDetails?.accountNumber || 'N/A'}</span>
                </div>
                <div>
                  <span className="text-gray-400 block font-medium">IFSC Code</span>
                  <span className="font-mono font-bold text-gray-900">{payout.bankDetails?.ifscCode || 'N/A'}</span>
                </div>
              </div>
            )}
            {payout.notes && (
              <p className="text-xs text-gray-500 italic mt-1">
                <span className="font-semibold text-gray-600">Requester note:</span> {payout.notes}
              </p>
            )}
          </div>
          ...
          {isPending ? (
              ...
              {action === 'approve' ? (
                  ...
                  <div>
                    ...
                    {proofImage ? (
                      ...
                      <label className="border-2 border-dashed border-gray-300 hover:border-emerald-500 rounded-xl p-4 flex flex-col items-center justify-center gap-1.5 cursor-pointer bg-white transition-colors">
                        ...
                        <span className="text-xs font-bold text-gray-700">Click to upload payment proof</span>
                ...
                <div className="space-y-3 bg-rose-50/40 border border-rose-100 rounded-2xl p-4">
                  ...
                  <p className="text-[11px] text-rose-600 font-medium">
                    ⚠️ Rejecting this request will immediately refund ₹{payout.amount.toLocaleString('en-IN')} back into the user's active wallet balance.
                  </p>
                </div>
              )}
            ...
            <div className="border border-gray-200 rounded-2xl p-4 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-gray-400 uppercase">Status Result</span>
                <span
              ...
              {payout.status === 'Approved' && (
                <div className="space-y-2 text-xs">
                  <div className="flex justify-between py-1 border-b border-gray-100">
                    ...
                    <span className="font-bold text-gray-900">{payout.paymentMethod || 'N/A'}</span>
                  </div>
                  {payout.referenceId && (
                    <div className="flex justify-between py-1 border-b border-gray-100">
                      <span className="text-gray-500">Reference / Txn ID:</span>
                      <span className="font-mono font-bold text-gray-900">{payout.referenceId}</span>
                    </div>
                  )}
                  {payout.paymentProofImage && (
                    <div className="pt-2">
                      <span className="text-gray-500 block mb-1 font-medium">Payment Proof Receipt:</span>
                      <div className="relative rounded-xl overflow-hidden border border-gray-200 bg-gray-50 max-w-xs shadow-sm">
                        <img
                          ...
                          onClick={() => window.open(payout.paymentProofImage, '_blank')}
                        />
                      </div>
                    </div>
                  )}
