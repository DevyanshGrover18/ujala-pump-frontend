...
export default function Sales() {
    ...
    const fetchDealers = async () => {
      try {
        const res = await axios.get(
          `${API_URL}/api/distributors/${user.distributor._id}/dealers`
        );
        setDealers(res.data);
      } catch (err) {
  ...
  const fetchSalesData = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}/api/sales/distributor-assigned-products?${params.toString()}`
      );
  ...
  const getWarrantyInfo = (product) => {
    ...
    console.log('distributorDistrict - ', distributorDistrict);
  ...
  const handleConfirmPermanentDelete = async () => {
    ...
    try {
      const response = await axios.delete(
        `${API_URL}/api/sales/distributor-sale-delete/${orderToDelete._id}`
      );
  ...
  const handleRevertAssignment = async () => {
    ...
    // Get the first selected product to extract dealerId
    const firstSelectedProduct = modalProducts.find((p) =>
      productIds.includes(p._id)
    );

    if (!firstSelectedProduct || !firstSelectedProduct.dealer?._id) {
      toast.error(
        'Could not determine dealer for selected products. Please ensure selected products have an assigned dealer.'
      );
      return;
    }

    const dealerId = firstSelectedProduct.dealer._id;

    // Validate if all selected products belong to the same dealer
    const allSameDealer = productIds.every((id) => {
      const product = modalProducts.find((p) => p._id === id);
      return product && product.dealer?._id === dealerId;
    });
    ...
    try {
      const response = await axios.put(
        `${API_URL}/api/distributor-dealer-products/revert-dealer-assign`,
        {
          productIds,
          dealerId,
        }
    ...
    <div className="p-4 lg:p-4 min-h-full">
      ...
      {isModelModalOpen && (
        <div className="fixed inset-0 z-60 flex items-center justify-center">
          ...
          <div className="bg-white z-70 w-full h-full flex flex-col">
