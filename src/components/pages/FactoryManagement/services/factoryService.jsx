...
export const deleteMultipleFactories = async (factoryIds) => {
  if (
    window.confirm(
      `Are you sure you want to delete ${factoryIds.length} selected factories?`
    )
  ) {
    try {
      ...
      toast.error(
        error.response?.data?.message || 'Error deleting selected factories'
      );
      throw error;
    }
  }
...
export const getNewOrdersCount = async (factoryId) => {
  try {
    const response = await axios.get(
      `${API_URL}/${factoryId}/new-orders-count`
    );
